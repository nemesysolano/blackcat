...
BEGIN
    ...
    price_pivots AS (
        SELECT 
			...
			("B" - LAG("B", 1) OVER (ORDER BY curr."TIMESTAMP")) + 
			("B" - LAG("B", 2) OVER (ORDER BY curr."TIMESTAMP")) Bb, 
        	((LEAD("B", 1) over (partition by curr."TICKER" order by curr."TIMESTAMP") - "B") +
            (LEAD("B", 2) over (partition by curr."TICKER" order by curr."TIMESTAMP") -  "B") + 
            (LEAD("B", 3) over (partition by curr."TICKER" order by curr."TIMESTAMP") -  "B"))  Bf,
