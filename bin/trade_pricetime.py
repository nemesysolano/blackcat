...
from qf.trade.fracdiff import trade_fracdiff, create_trade_dataset
import redis
...
def main(train_stats_folder, test_results_folder, model_name, max_leverage, platform_commission, initial_capital):
    ...
    with engine.connect() as connection:        
        for quote_name in quotes:
            if quote_name not in train_stats.index:
                print(f"Skipping {quote_name} as it is not present in training stats.")
                continue
...
if __name__ == "__main__": 
    train_stats_folder =  os.path.join(os.getcwd(), "train-results")
    test_results_folder = os.path.join(os.getcwd(), "test-results")
    max_leverage = 10
    initial_capital = 50_000
    platform_commission = 6

    main(train_stats_folder, test_results_folder, "price-time-wavelet-direction",  max_leverage, platform_commission, initial_capital)