...
def predictor(connection, quote_name, model_name, X_test):
...
if __name__ == "__main__":
    ...
    with engine.connect() as connection:
        for quote_name in quotes:
            ...
            tradable = discriminators[discriminator](quote_name) # check_if_tradable(price_direction) and check_if_tradable(volume_direction) and
            ...
            if tradable:
                ...
                # Create datasets
                _, _, X_price_time_test, _, _, _, _ = create_local_datasets(price_time_wavelet_direction(connection, quote_name, lookback_periods))
                _, _, X_volume_time_test, _, _, _, _ = create_local_datasets(volume_time_wavelet_direction(connection, quote_name, lookback_periods))
                _, _, X_force_test, _, _, _, _ = create_local_datasets(price_time_wavelet_force(connection, quote_name, lookback_periods))
