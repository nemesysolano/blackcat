...
int calculate_fractional_signal(    
    ...
    // 4. Structural States
    // THE FIX: Use std::abs() on the original doubles (thrust_signal/energy_signal), NOT the int signs.
    // Also keeping your intended commented asymmetric bound logic: (y > upper_f_bound)
    bool reversion_trigger = (y > upper_f_bound) && std::abs(thrust_signal) > std::abs(energy_signal); 
    bool alignment_trigger = (y >= lower_f_bound) && (y <= upper_f_bound) && std::abs(thrust_signal) <= std::abs(energy_signal); 

    // 5. Execution Logic
    if(Lambda_ratio > L_ratio && order < 1) {
        if (reversion_trigger && reversion_align) {
            if (sign_L > 0) return MEAN_REVERSION_SHORT; 
...
int calculate_fractional_relaxed_signal(    
    double L, double L_hat, double Lambda, double Lambda_hat, double y, double y_mean, double y_std, double order, double energy_signal, double thrust_signal
) { 
    if (std::isnan(y_mean) || std::isnan(y_std) || std::isnan(y)) {
        return STALL;
    ...
    // 2. Verify exact Phase Alignment (Signs)
    int sign_L = sign(L + L_hat); 
    int sign_Lambda = sign(Lambda + Lambda_hat);
    ...
    // GATE 1: Trend alignment demands Conviction/Thrust (e^lambda)
    bool all_align = sign_L == sign_Lambda && (sign_Lambda == sign_thrust);
    
    // GATE 2: Reversal alignment demands Energy/Inflection (|Lambda|)
    bool reversion_align = sign_L != sign_Lambda && (sign_Lambda == sign_energy);
    ...
    // 4. Structural States
    // THE FIX: applied here as well to keep relaxed logic perfectly synchronized
    bool reversion_trigger = (y > upper_f_bound) && std::abs(thrust_signal) > std::abs(energy_signal); 
    bool alignment_trigger = (y >= lower_f_bound) && (y <= upper_f_bound) && std::abs(thrust_signal) <= std::abs(energy_signal); 
...
int calculate_fractional_relaxed_signal_cy(
    double L, double L_hat, double Lambda, double Lambda_hat, double y, double y_mean, double y_std, double order, double energy_signal, double thrust_signal) {
    return calculate_fractional_relaxed_signal(L, L_hat, Lambda, Lambda_hat, y, y_mean, y_std, order, energy_signal, thrust_signal);
}